 ...
 * `Schema` from `cratebase typegen` (or the older
 * `@cratebase/schema-codegen`) to `createClient` for typed
 * `client.collection(name)`. The constraint is deliberately
...
/** A per-collection map shaped like `Schema` itself — what `SchemaCreate`/
 * `SchemaUpdate` (also generated by `cratebase typegen`) are. Passed as
 * `createClient`'s 2nd/3rd type params, they type every
 * `collection(name).create()`/`.update()` call to that collection's own
 * `<Name>Create`/`<Name>Update` input shape (required fields respected,
 * system fields excluded) instead of the `Partial<Record>` default —
 * see `CollectionService`'s module doc. */
export type SchemaInputMap<S extends AnySchema> = { [K in keyof S]: Record<string, unknown> };

export class CratebaseClient<
  S extends AnySchema = Record<string, RecordModel>,
  SC extends SchemaInputMap<S> = { [K in keyof S]: Partial<S[K]> },
  SU extends SchemaInputMap<S> = { [K in keyof S]: Partial<S[K]> },
> {
  ...
  collection<K extends keyof S & string>(name: K): CollectionService<S[K], SC[K], SU[K]>;
...
export function createClient<
  ...
  SC extends SchemaInputMap<S> = { [K in keyof S]: Partial<S[K]> },
  SU extends SchemaInputMap<S> = { [K in keyof S]: Partial<S[K]> },
>(baseUrl: string, options: CreateClientOptions = {}): CratebaseClient<S, SC, SU> {
  return new CratebaseClient<S, SC, SU>(baseUrl, options);
}
