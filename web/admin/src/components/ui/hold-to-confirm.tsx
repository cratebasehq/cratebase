...
/* The two label faces cross-fade on a spring rather than a duration so the
 * swap tracks the fill instead of running on its own clock. */
...
function HoldToConfirm({
  ...
  return (
    <button
      ...
      className={cn(
        "relative isolate inline-grid h-control-sm cursor-pointer place-items-center overflow-hidden rounded-md border border-destructive/40 px-3 text-sm font-medium text-destructive transition-colors select-none outline-none focus-visible:border-destructive focus-visible:ring-3 focus-visible:ring-destructive/20 dark:focus-visible:ring-destructive/40",
    ...
    >
      ...
      <motion.span
        aria-hidden
        style={{ clipPath }}
        className="absolute inset-0 grid place-items-center bg-destructive px-3 text-destructive-foreground"
      >
        <Faces committed={committed} confirmLabel={confirmLabel}>
          {children}
        </Faces>
...
export { HoldToConfirm, useHoldToConfirm }
export type { HoldPhase, UseHoldToConfirmOptions, HoldToConfirmProps }
