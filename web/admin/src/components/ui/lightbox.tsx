...
const EXIT = { duration: 0.2, ease: EASE_IN_STRONG } as const
...
// --radius is 0.625rem; the image corner is animated as a number, so the
// token value is carried here rather than as a class.
const REST_RADIUS = 10

const CHROME_BUTTON =
  "grid size-control-md place-items-center rounded-md border border-border bg-card/80 text-foreground backdrop-blur-sm transition-colors outline-none hover:border-border-strong hover:bg-card focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50"
...
>({
  ...
  // `settled` lags `step` so the screen-reader zoom readout speaks once the
  // gesture stops rather than on every wheel tick.
  ...
  // Zooming keeps the point under the cursor fixed: solve for the pan that
  // leaves (cx, cy) mapping to the same image coordinate at the new scale.
  ...
  const release = (e: ReactPointerEvent) => {
    ...
    // A tap on the backdrop (not the image, not a drag, not zoomed) dismisses.
  ...
  useEffect(() => {
    const frame = frameRef.current
    if (!frame) return
    // Non-passive so the page behind the lightbox never scrolls away.
...
function Stage({
  ...
  const fr = useMotionValue(REST_RADIUS)
  ...
  /* Where the image flies from and back to. With an `originRef` the opening
   * is measured off the thumbnail so the two frames are the same object;
   * without one it is a plain rise-and-fade. */
  const landing = useCallback((): Landing => {
    ...
    if (frame && content && origin && content.offsetWidth > 0) {
      ...
      if (o.width > 0) {
        ...
        const rad =
          Number.parseFloat(getComputedStyle(origin).borderTopLeftRadius) ||
          REST_RADIUS
        return {
    ...
    return { dx: 0, dy: 10, s: 0.97, o: 0, r: REST_RADIUS }
  ...
  // The image carries its own pan/zoom; unwind it so the exit flight starts
  // from the fitted frame rather than from wherever the user left it.
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        // Zoomed in, Escape belongs to the frame: it returns home first.
        if (zoomed) return
        e.preventDefault()
        onClose()
        return
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          "absolute inset-0 overflow-hidden outline-none select-none focus-visible:ring-3 focus-visible:ring-ring/50 focus-visible:ring-inset",
      ...
      >
        <motion.div
          ...
          transition={
            reduced
              ? { duration: 0 }
              : { filter: { duration: 0.35, ease: EASE_OUT_STRONG } }
          }
      ...
      >
        <p
          id={titleId}
          className="pointer-events-auto max-w-[65%] truncate rounded-md border border-border bg-card/80 px-2.5 py-1.5 text-xs text-muted-foreground backdrop-blur-sm"
        >
          {caption ?? alt}
        </p>
        <div className="pointer-events-auto flex items-center gap-2">
          ...
          >
            {/* Both glyphs share one grid cell and cross-fade, so the button
                never reflows as the label flips between zoom in and out. */}
...
function Lightbox(props: LightboxProps) {
  // Portalled to <body>, so mount is deferred one tick to stay SSR-safe.
