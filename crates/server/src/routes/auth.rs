...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/collections/{collection}/auth-with-password", post(record_login))
        .route("/collections/{collection}/auth-refresh", post(record_refresh))
}
...
async fn admin_login(State(app): State<AppState>, Json(body): Json<PasswordLogin>) -> ApiResult<Json<Value>> {
    ...
    let token = issue_token(&admin.id, TokenKind::Admin, "", &app.config.auth_secret, app.config.admin_token_ttl_seconds)
        .map_err(|e| ApiError(AppError::Internal(e.to_string())))?;
    Ok(Json(json!({ "token": token, "admin": admin })))
}

async fn admin_refresh(State(app): State<AppState>, CurrentAuth(auth): CurrentAuth) -> ApiResult<Json<Value>> {
    let ctx = auth.ok_or_else(|| ApiError(AppError::Unauthorized("missing or invalid token".into())))?;
    if !ctx.is_superuser {
        return Err(ApiError(AppError::Unauthorized("missing or invalid token".into())));
    }
...
async fn record_login(
    ...
    let Some((id, password_hash)) = records::find_auth_credentials(&app.db, &collection, &body.email).await? else {
        return Err(invalid_credentials());
    };
